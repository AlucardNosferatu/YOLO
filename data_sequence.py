...
class SequenceForCelebA(Sequence):
    def __init__(self, model, annotation, img_dir, target_size, batch_size, shuffle=True):
        self.model = model
        self.data_sets = []
        ...
        if self.model is 'train':
            bounding_boxes = bounding_boxes[:int(0.8 * count)]
        elif self.model is 'val':
            bounding_boxes = bounding_boxes[int(0.8 * count):]
        for box in bounding_boxes:
            info = [a for a in box.strip().split(' ') if len(a) > 0]
            x = int(info[1])
            y = int(info[2])
            w = int(info[3])
            h = int(info[4])
            img_path = os.path.join(img_dir, info[0])
            self.data_sets.append({"xywh": [x, y, w, h], "jpg": img_path})
        self.image_size = target_size[0:2]
        self.batch_size = batch_size
        self.indexes = np.arange(len(self.data_sets))
        self.shuffle = shuffle
